...
local function AuxClassifier1(fs_start, nClasses)
  fs = {128, 128*4*4, 1024}; fs[0] = fs_start
 
  local model = nn.Sequential()
  model:add(Avg(5,5,4,4):ceil())
  model:add(addConv(fs[0], fs[1], 1, 1, 1, 1, 0, 0))
  model:add(nn.View(fs[2]):setNumInputDims(3))

  local lin = nn.Linear(fs[2], fs[3])
  ...
  model:add(lin)
  model:add(BatchNorm(fs[3]))
  model:add(nn.ReLU())
  model:add(nn.Linear(fs[3], nClasses))
  return model
end

local function AuxClassifier2(fs_start, nClasses)
  fs = {256, 256*2*2, 1024}; fs[0] = fs_start

  local model = nn.Sequential()
  model:add(Avg(5,5,3,3):ceil())
  model:add(addConv(fs[0], fs[1], 1, 1, 1, 1, 0, 0))
  model:add(nn.View(fs[2]):setNumInputDims(3))
...
local function Classifier(fs_start, nClasses)
   local nFeatures = fs_start -- set final channels
   local model = nn.Sequential()
...
local function createModel(opt)
   ...
   local auxcls1 = AuxClassifier1(fs, nClasses)
   ...
   -- Add auxiliary classifier
   local auxcls2 = AuxClassifier2(fs, nClasses)
   ...
   local cls = Classifier(fs, nClasses)
   path3:add(cls)
